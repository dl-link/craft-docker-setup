...
# ══════════════════════════════════════════════════════════
# Craft Agent Server — Docker build
# Auto-generated by setup.sh
# ══════════════════════════════════════════════════════════
FROM oven/bun:1.3-slim

RUN apt-get update && apt-get install -y --no-install-recommends \
    git ca-certificates curl unzip python3 && \
...
# Build MCP servers (session-mcp-server + bridge-mcp-server)
RUN bunx esbuild packages/session-mcp-server/src/index.ts \
      --bundle --platform=node --format=esm --target=esnext \
      --outfile=packages/session-mcp-server/dist/index.js && \
    bunx esbuild packages/bridge-mcp-server/src/index.ts \
      --bundle --platform=node --format=esm --target=esnext \
      --outfile=packages/bridge-mcp-server/dist/index.js 2>/dev/null || true
